...
def playableWord(word, letters):
    copyHand = letters
#    print(f"Testing word {word.word}")
    for char in word.word:        # For every character in the word
#        print(f"Copyhand: {copyHand}", end='')
#        print(f"    --- Testing chracter: {char}")
        if char not in copyHand:   # If the current character is not in the hand, return false
#            print("\n\n")
            return False
        else:
            copyHand.remove(char)
